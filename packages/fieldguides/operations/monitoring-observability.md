...
export async function getHealthStatus(): Promise<HealthStatus> {
  ...
  const results = checks.map((c, i) => ({
    ...healthChecks[i],
    ...(c.status === 'fulfilled' ? c.value : { status: 'unhealthy', error: c.reason }),
  }));
...
export class DORAMetrics {
  async calculateLeadTime(commitSha: string, environment: string): Promise<number> {
  ...
  async getDeploymentFrequency(environment: string, days: number = 30): Promise<number> {
  ...
  async getChangeFailureRate(environment: string, days: number = 30): Promise<number> {
    const deployments = await this.getDeployments(environment, days);
