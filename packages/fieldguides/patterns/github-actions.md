    ...
    - name: Cache dependencies
      ...
      with:
        path: |
          ...
          node_modules
          .next/cache
        key: ${{ runner.os }}-${{ steps.detect-pm.outputs.manager }}-${{ hashFiles(format('**/{0}', steps.detect-pm.outputs.lockfile)) }}
        restore-keys: |
      ...
      - name: Cache with fallback
        ...
        with:
          path: |
            node_modules
            .build-cache
          key: cache-${{ steps.cache-keys.outputs.deps-hash }}-${{ steps.cache-keys.outputs.src-hash }}
          restore-keys: |
