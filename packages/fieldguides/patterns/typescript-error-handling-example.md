...
export class UserService {
  ...
  private validateUserUpdates(
    ...
    if (updates.email && !z.string().email().safeParse(updates.email).success) {
      errors.push(Errors.validation('email', 'Invalid email format', updates.email));
    }
    
    if (updates.name && updates.name.trim().length === 0) {
      errors.push(Errors.validation('name', 'Name cannot be empty', updates.name));
    }
...
export const UserId = {
  parse(id: string): Result<UserId, ValidationError> {
    if (!id || !id.match(/^user_[a-zA-Z0-9]{16}$/)) {
      return Result.failure(
        Errors.validation('id', 'Invalid user ID format')
      );
    }
...
export class SecureUserService {
  ...
  async authenticate(
    apiKey: ApiKey
  ): AsyncResult<SessionId, ApiError> {
