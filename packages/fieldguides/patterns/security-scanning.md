  ...
  secret-scan:
    ...
    steps:
      ...
      - name: Aggregate results
        if: steps.trufflehog.outcome == 'failure' || steps.gitleaks.outcome == 'failure' || steps.detect-secrets.outcome == 'failure'
        run: |
...
jobs:
  license-check:
    ...
    steps:
      ...
      - name: Comment on PR
        if: github.event_name == 'pull_request' && (env.LICENSE_ISSUES == 'true' || env.LICENSED_ISSUES == 'true')
        uses: actions/github-script@v7
        with:
          script: |
            const fs = require('fs');
