...
export async function teardownTestDatabase(prisma: PrismaClient) {
  ...
  const adminClient = new PrismaClient({
    datasources: {
      db: { url: `postgresql://${url.username}:${url.password}@${url.host}/postgres` },
    },
  });
...
describe('GitHubService Integration', () => {
  ...
  it('should handle API errors', async () => {
    ...
    await expect(githubService.getUser('nonexistent'))
...
describe('API Client Integration', () => {
  it('should handle timeout scenarios', async () => {
    ...
    await expect(client.get('https://api.slow.com/data'))
