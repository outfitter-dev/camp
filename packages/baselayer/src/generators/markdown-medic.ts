import type { OutfitterConfig, MdMedicConfig } from '../types/index.js';

/**
 * Generates markdown-medic configuration from OutfitterConfig
 */
export function generateMarkdownMedicConfig(config: OutfitterConfig): MdMedicConfig {
  const { strictness, overrides } = config;

  // Base configuration - start with preset based on strictness
  const baseConfig: MdMedicConfig = {
    ...
    // Custom rules from markdown-medic
  ...
  if (overrides?.['markdown-medic']) {
    return {
      ...baseConfig,
      ...overrides['markdown-medic'],
      // Special handling for arrays that should be merged rather than replaced
      terminology: Array.isArray(overrides['markdown-medic'].terminology)
        ? [...(baseConfig.terminology || []), ...overrides['markdown-medic'].terminology]
        : baseConfig.terminology,
      customRules: Array.isArray(overrides['markdown-medic'].customRules)
        ? [...(baseConfig.customRules || []), ...overrides['markdown-medic'].customRules]
        : baseConfig.customRules,
      ignores: Array.isArray(overrides['markdown-medic'].ignores)
        ? [...(baseConfig.ignores || []), ...overrides['markdown-medic'].ignores]
        : baseConfig.ignores,
    };
  }
 ...
 */
export function generateMarkdownMedicConfigContent(config: OutfitterConfig): string {
  const mdConfig = generateMarkdownMedicConfig(config);
