...
export async function generateOxlintConfig(): Promise<Result<void, Error>> {
  try {
    console.log(pc.blue('→ Setting up Oxlint...'));
    
    // Check if ESLint config exists for migration
    const hasEslintConfig = await detectEslintConfig();
    
    if (hasEslintConfig) {
      console.log(pc.yellow('  Found ESLint config, attempting migration...'));
      try {
        ...
        console.log(pc.yellow('  Migration failed, creating new config instead'));
    ...
    console.log(pc.green('✓ Oxlint configured successfully'));
