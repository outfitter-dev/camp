  ...
  .addCommand(
    new Command('export')
      ...
      .action(async (options) => {
        ...
        console.log(chalk.green('✓') + ' Exported packlist to ' + chalk.cyan(options.output));
      })
  )
  .addCommand(
    new Command('import')
      ...
      .action(async (file: string) => {
        ...
        console.log(chalk.green('✓') + ` Imported ${packlist.supplies.length} supplies from ${chalk.cyan(packlist.name)}`);
      })
  );