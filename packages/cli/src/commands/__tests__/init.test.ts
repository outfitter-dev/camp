...
describe('init command', () => {
  ...
  it('successfully initializes project with valid name and template', async () => {
    (prompts as jest.Mock).mockResolvedValue({ projectName: 'test-app', template: 'default' });
    (fs.pathExists as jest.Mock).mockResolvedValue(false);
    const ensureDirSpy = (fs.ensureDir as jest.Mock).mockResolvedValue(undefined);
    const writeFileSpy = (fs.writeFile as jest.Mock).mockResolvedValue(undefined);

    await expect(init({ name: 'test-app', template: 'default' })).resolves.not.toThrow();
    ...
    expect(logger.info).toHaveBeenCalledWith(expect.stringContaining('Project initialized'));
  });
  ...
  it('throws an error for invalid template option', async () => {
    await expect(init({ name: 'app', template: 'nonexistent' })).rejects.toThrow(
      'Invalid template: nonexistent',
    );
  });

  it('handles filesystem errors gracefully', async () => {
    (prompts as jest.Mock).mockResolvedValue({ projectName: 'app', template: 'default' });
    (fs.pathExists as jest.Mock).mockResolvedValue(false);
    (fs.ensureDir as jest.Mock).mockRejectedValue(new Error('Disk full'));

    await expect(init({ name: 'app', template: 'default' })).rejects.toThrow('Disk full');
  ...
  it('prompts user when no options are provided and proceeds with input', async () => {
    (prompts as jest.Mock).mockResolvedValue({ projectName: 'interactive-app', template: 'default' });
    (fs.pathExists as jest.Mock).mockResolvedValue(false);
