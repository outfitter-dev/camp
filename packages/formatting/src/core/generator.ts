...
const generatePrettierConfig: ConfigGenerator = hasGenerateFunction(prettierConfigModule)
  ? prettierConfigModule.generate
  : (prettierConfigModule as ConfigGenerator);
...
function generateBiomeConfigFile(preset: PresetConfig): Result<GeneratedConfig, Error> {
  try {
    ...
    const config = generateBiomeConfig(biomeOptions);
