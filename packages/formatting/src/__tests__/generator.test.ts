...
describe('generator', () => {
  describe('generateFormatterConfig', () => {
    it('should generate prettier config', async () => {
      ...
      if (result.success) {
        expect(result.data.path).toBe('.prettierrc.json');
    ...
    it('should generate biome config', async () => {
      ...
      expect(result.success).toBe(true);
      if (result.success) {
        expect(result.data.path).toBe('biome.json');
    ...
    it('should fail for unknown formatter', async () => {
      const result = await generateFormatterConfig('unknown' as any, standard);
      
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.code).toBe('UNKNOWN_FORMATTER');
      }
    });
  });
